...
export class ProtocolTemplateModalComponent implements OnInit {
  ...
  preview(): void {
    // Preview functionality removed - preview is now shown when generating protocols
    this.#notificationService.showError(
      'Preview is available when generating protocols, not when creating templates.'
    );
  }
}
